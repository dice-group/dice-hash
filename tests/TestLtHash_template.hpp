...
template <typename H>
struct LtHashTest {
	static constexpr std::array<std::byte, 1> obj1{static_cast<std::byte>('a')};
	static constexpr std::array<std::byte, 1> obj2{static_cast<std::byte>('b')};
	static constexpr size_t checksum_len = H::element_count / H::elements_per_uint64 * sizeof(uint64_t);
...
TEMPLATE_TEST_CASE("Test LtHash using " DICE_HASH_TEST_LTHASH_INSTRUCTION_SET, "[DiceHash]", DICE_HASH_TEST_LTHASH_CONFIGS) {
	...
	SECTION("empty") {
		...
		CHECK(checksum_len == H::checksum_len);
		auto checksum = h.checksum();
		CHECK(checksum_len == checksum.size());
		CHECK(std::string(checksum_len * 2, '0') == to_hex(checksum));
	}
	...
	SECTION("add chaining") {
		H h1;
		h1.add(T::obj1);
		h1.add(T::obj2);
		H h2;
		h2.add(T::obj1).add(T::obj2);
		
		// add('a'); add('b') == add('a').add('b')
		CHECK(h1 == h2);
	}

	SECTION("add different objects") {
		H h1;
		H h2;
		h1.add(T::obj1);
		h2.add(T::obj2);
	...
	SECTION("add and remove same object") {
		H h;
		h.add(T::obj1);
		h.remove(T::obj1);
	...
	SECTION("add with initial checksum") {
		H h1;
		h1.add(T::obj1);
		h1.add(T::obj2);
	...
	SECTION("add object random shuffle") {
		...
		for (int i = 0; i < objectCount; i++) {
			...
			objects.push_back(make_random_data(objectSize));
		}
	...
	SECTION("add and remove random") {
		...
		std::vector<std::vector<std::byte>> checksums;
		checksums.push_back(std::vector<std::byte>{h.checksum().begin(), h.checksum().end()});
		...
		CHECK(std::ranges::equal(h.checksum(), checksums.back()));
		for (int i = static_cast<int>(objects.size() - 1); i >= 0; i--) {
			h.remove(objects[i]);
			CHECK(std::ranges::equal(h.checksum(), checksums[i]));
		}
		CHECK(h == T::empty_hash);
	}

	SECTION("set checksum") {
		H h1;
		h1.add(T::obj1);
		auto checksum = h1.checksum();
		H h2{checksum}; // copy version
		CHECK(h1 == h2);

		H h4;
		h4.set_checksum(checksum); // copy version
		CHECK(h1 == h4);
	}

	SECTION("set checksum fail") {
		if constexpr (H::needs_padding) {
			...
			std::array<std::byte, H::checksum_len> new_checksum;
			std::ranges::copy(checksum, new_checksum.begin());

			uint64_t *ptr = reinterpret_cast<uint64_t *>(new_checksum.data());
			*ptr = 0xFFFFFFFFFFFFFFFFULL;
			CHECK_THROWS(h1.set_checksum(new_checksum));
		}
	}

	SECTION("add object with known value") {
		auto obj3 = as_bytes(std::span{"hello", 5});
		H h;
		h.add(T::obj1);
		h.add(T::obj2);
		h.add(obj3);
		h.remove(T::obj2);
		if (H::element_bits == 16 && H::element_count == 1024) {
		...
		} else if (H::element_bits == 20 && H::element_count == 1008) {
			std::string_view expected_checksum =
			...
			CHECK(expected_checksum == to_hex(h.checksum()));
		} else if (H::element_bits == 32 && H::element_count == 1024) {
	...
	SECTION("key too short") {
		H h1;
		auto short_key = as_bytes(std::span{"0123456789abcde", 15});
		CHECK_THROWS(h1.set_key(short_key));
	}

	SECTION("key too long") {
		H h1;
		auto long_key = as_bytes(std::span{"0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef0", 65});
		CHECK_THROWS(h1.set_key(long_key));
	}

	SECTION("add with different keys fail") {
		H h1;
		h1.add(T::obj1);

		H h2;
		h2.set_key(as_bytes(std::span{"0123456789abcdef"}));
		h2.add(T::obj1);
		CHECK_THROWS(h1.combine_add(h2));
	}

	SECTION("subtract with different keys fail") {
		H h1;
		h1.add(T::obj1);
		...
		h2.set_key(as_bytes(std::span{"0123456789abcdef"}));
		h2.add(T::obj1);
		CHECK_THROWS(h1.combine_remove(h2));
	}

	SECTION("keyed LtHashs same key equal") {
		auto key = as_bytes(std::span{"0123456789abcdef"});
		H h1;
		h1.set_key(key);
		h1.add(T::obj1);
		h1.add(T::obj2);
	...
	SECTION("keyed LtHashes different keys not equal") {
		auto key = as_bytes(std::span{"0123456789abcdef"});
		H h1;
		h1.set_key(key);
		h1.add(T::obj1);
		...
		auto key2 = as_bytes(std::span{"1123456789abcdef"});
		H h2;
		h1.set_key(key2);
		h2.add(T::obj1);
		h2.add(T::obj2);
