...
namespace Dice::tests::hash {
    ...
    */
	...
	TEST_CASE("dice_hash_invertible_combine can be called with any number of size_t", "[DiceHash]") {
		...
		std::size_t c = 7;
		std::size_t d = 42;
		Dice::hash::dice_hash_invertible_combine(a, b, c, d);
	}

	TEST_CASE("dice_hash_invertible_combine can be inverted", "[DiceHash]") {
		std::size_t a = 3;
		std::size_t b = 4;
		REQUIRE(a == Dice::hash::dice_hash_invertible_combine(a, b, a, a, b));
	}
