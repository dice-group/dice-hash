...
namespace dice::hash::lthash {
	...
	struct MathEngine_AVX2 {
		...
		static void add(std::span<std::byte, DstExtent> dst, std::span<std::byte const, SrcExtent> src) noexcept {
			...
			assert(reinterpret_cast<uintptr_t>(dst.data()) % alignof(__m256i) == 0);
			assert(reinterpret_cast<uintptr_t>(src.data()) % alignof(__m256i) == 0);
			...
			if constexpr (!Bits::needs_padding) {
				...
				for (size_t ix = 0; ix < dst.size() / sizeof(__m256i); ++ix) {
					auto const ai = _mm256_load_si256(dst256 + ix);
					auto const bi = _mm256_load_si256(src256 + ix);

					if constexpr (Bits::bits_per_element == 16) {
						_mm256_store_si256(dst256 + ix, _mm256_add_epi16(ai, bi));
					} else { // Bits::bits_per_element == 32
						_mm256_store_si256(dst256 + ix, _mm256_add_epi32(ai, bi));
					}
				}
			} else {
				...
				for (size_t ix = 0; ix < dst.size() / sizeof(__m256i); ++ix) {
					...
					_mm256_store_si256(dst256 + ix, _mm256_and_si256(_mm256_add_epi64(ai, bi),
																	 mask));
				}
			}
		}
		...
		static void sub(std::span<std::byte, DstExtent> dst, std::span<std::byte const, SrcExtent> src) noexcept {
			...
			if constexpr (!Bits::needs_padding) {
				...
				for (size_t ix = 0; ix < dst.size() / sizeof(__m256i); ++ix) {
					...
					if constexpr (Bits::bits_per_element == 16) {
						_mm256_store_si256(dst256 + ix, _mm256_sub_epi16(ai, bi));
					} else { // Bits::bits_per_element == 32
						_mm256_store_si256(dst256 + ix, _mm256_sub_epi32(ai, bi));
					}
				}
			} else {
				__m256i const mask = _mm256_set1_epi64x(Bits::data_mask);
				...
				for (size_t ix = 0; ix < dst.size() / sizeof(__m256i); ++ix) {
					...
					_mm256_store_si256(dst256 + ix, _mm256_and_si256(_mm256_add_epi64(ai, inv_bi),
																	 mask));
				}
			}
		}
		...
		static void clear_padding_bits(std::span<std::byte, Extent> data) noexcept requires (Bits::needs_padding) {
			...
			if constexpr (Bits::needs_padding) {
				...
				for (size_t ix = 0; ix < data.size() / sizeof(__m256i); ++ix) {
					auto const val = _mm256_load_si256(data256 + ix);
					_mm256_store_si256(data256 + ix, _mm256_and_si256(val, mask));
				}
			}
		}
	};
