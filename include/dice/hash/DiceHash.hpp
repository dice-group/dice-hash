...
namespace dice::hash {
	...
	private:
		...
		static std::size_t dice_hash_ordered_container(Container const &container) noexcept {
			typename Policy::HashState hash_state(container.size(), Policy::SeedValue);
		...
		static std::size_t dice_hash_unordered_container(Container const &container) noexcept {
			...
			for (auto const &it : container) {
				h = Policy::hash_invertible_combine({dice_hash(it)}, h);
			}
			return h;
		}
		...
		static std::size_t dice_hash_tuple(Tuple &tuple, std::index_sequence<ids...> const &) {
			return Policy::hash_combine({dice_hash(std::get<ids>(tuple))...}, Policy::SeedValue);
		}
	...
	public:
		...
		static std::size_t dice_hash(T const &fundamental) noexcept {
			return Policy::hash_fundamental(fundamental, Policy::SeedValue);
		}
		...
		static std::size_t dice_hash(std::basic_string<CharT> const &str) noexcept {
			return Policy::hash_bytes(str.data(), sizeof(CharT) * str.size(), Policy::SeedValue);
		}
		...
		static std::size_t dice_hash(std::basic_string_view<CharT> const &sv) noexcept {
			return Policy::hash_bytes(sv.data(), sizeof(CharT) * sv.size(), Policy::SeedValue);
		}
		...
		static std::size_t dice_hash(T *ptr) noexcept {
			return Policy::hash_fundamental(ptr, Policy::SeedValue);
		}
        ...
        */
		...
		static std::size_t dice_hash(std::array<T, N> const &arr) noexcept {
			if constexpr (is_fundamental<T>) {
				return Policy::hash_bytes(arr.data(), sizeof(T) * N, Policy::SeedValue);
			} else {
		...
		static std::size_t dice_hash(std::vector<T> const &vec) noexcept {
			if constexpr (is_fundamental<T>) {
				...
				return Policy::hash_bytes(vec.data(), sizeof(T) * vec.size(), Policy::SeedValue);
			} else {
		...
		static std::size_t dice_hash(std::span<T, Extent> const &span) noexcept {
			if constexpr (is_fundamental<T>) {
				return Policy::hash_bytes(span.data(), span.size_bytes(), Policy::SeedValue);
			} else {
		...
		static std::size_t dice_hash(std::pair<T, V> const &p) noexcept {
			return dice_hash_tuple(p, std::make_index_sequence<2>());
		}
	...
	struct DiceHash : private Policy {
		...
		static std::size_t hash_combine(std::initializer_list<size_t> hashes) noexcept {
			return Policy::hash_combine(hashes, Policy::SeedValue);
		}
		...
		static std::size_t hash_invertible_combine(std::initializer_list<size_t> hashes) noexcept {
			return Policy::hash_invertible_combine(hashes, 0);
		}
